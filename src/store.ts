...
import type { Store, Transaction, Month, MonthYear } from "./types"
...
export const envelopeBalances = createMemo(() => {
  ...
  for (const nm of Object.keys(state.envelopes)) {
    const activity = state.transactions
      .filter(
        (txn) =>
          txn.envelope == nm && dateToMonthYear(txn.date) == state.currentMonth
      )
...
export const setIncMonth = () => setState("currentMonth", incMonth)
export const setDecMonth = () => setState("currentMonth", decMonth)
